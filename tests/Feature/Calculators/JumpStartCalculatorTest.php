...
it('ranks tumbling athletes for JumpStart team correctly', function () {
    ...
    $highestLevel10_11_12 = $rankings->first(fn (RankedAthlete $ra) => 
        $ra->athlete->getId() === 2); // Athlete 2 has highest qualification score
    expect($highestLevel10_11_12)->not->toBeNull();
    
    // Verify the highest scoring Level 10 10U athlete is included
    $highestLevel10_10U = $rankings->first(fn (RankedAthlete $ra) => 
        $ra->athlete->getId() === 4); // Athlete 4 has highest qualification score
    expect($highestLevel10_10U)->not->toBeNull();
    
    // Verify a level 9 athlete is included
    $anyLevel9 = $rankings->first(fn (RankedAthlete $ra) => 
        in_array($ra->athlete->getId(), [5, 6, 7, 8]));
    expect($anyLevel9)->not->toBeNull();
    
    // Verify a level 8 athlete is included
    $anyLevel8 = $rankings->first(fn (RankedAthlete $ra) => 
        in_array($ra->athlete->getId(), [9, 10, 11, 12]));
...
it('ranks double-mini athletes for JumpStart team correctly', function () {
    ...
    foreach ($rankings as $athlete) {
        ...
        if (in_array($id, $level10_11_12Athletes)) $hasLevel10_11_12 = true;
        if (in_array($id, $level10_10uAthletes)) $hasLevel10_10u = true;
        if (in_array($id, $level9_11_12Athletes)) $hasLevel9_11_12 = true;
        if (in_array($id, $level9_9_10Athletes)) $hasLevel9_9_10 = true;
        if (in_array($id, $level8_11_12Athletes)) $hasLevel8_11_12 = true;
        if (in_array($id, $level8_9_10Athletes)) $hasLevel8_9_10 = true;
    }
